...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'learner_accessibility' => 'required|in:paid,free',
        ...
        try {
            ...
            // Generate UUID - automatically by model boot method
            $course->uuid = Str::uuid();

            // Basic Information
            $course->user_id = Auth::id();
            $course->instructor_id = $request->instructor_id;
            $course->course_type = 1; // 1=general
            $course->organization_id = 1;

            // Title & Description
            $course->title = $request->title;
            $course->subtitle = $request->subtitle;
            $course->description = $request->description;
            $course->feature_details = $request->feature_details;

            // Pricing
            $course->price = $request->price ?? 0;
            $course->old_price = $request->old_price ?? null;
            $course->learner_accessibility = $request->learner_accessibility;

            // Handle Image Upload
            if ($request->hasFile('image')) {
                $course->image = $this->saveImage('course', $request->image);
            }

            // Default values for other fields
            $course->intro_video_check = 1; // Default normal video
            $course->is_subscription_enable = 1;
            $course->private_mode = 0;
            $course->drip_content = 1; // Show All
            $course->access_period = null;

            // Generate Slug
            $course->slug = Str::slug($request->title) . '-' . uniqid();

            // SEO Fields
            $course->meta_title = $request->meta_title ?? $request->title;
